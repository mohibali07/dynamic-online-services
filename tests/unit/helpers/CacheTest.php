...
class CacheTest extends TestCase
{
    public function setUp(): void
    {
        ...
        if (!class_exists(Cache::class)) {
            require_once dirname(__DIR__, 3) . '/includes/helpers/Cache.php';
        }
    }
    ...
    public function test_cache_clear_pattern(): void
    {
        ...
        /** @var array $transients_cast */
        $transients_cast = $transients;
        $wpdb->shouldReceive('get_col')
            ->once()
            ->andReturn($transients_cast);
    ...
    public function test_cache_clear_all(): void
    {
        ...
        /** @var array $all_transients */
        $all_transients = ['_transient_dynos_test'];
        $wpdb->shouldReceive('get_col')
            ->once()
            ->andReturn($all_transients);
    ...
    public function test_cache_get_terms(): void
    {
        ...
        /** @var array $terms_cast */
        $terms_cast = $terms;
        \WP_Mock::userFunction('get_terms')
            ->once()
            ->andReturn($terms_cast);
    ...
    public function test_cache_get_posts(): void
    {
        ...
        /** @var array $post_ids_cast */
        $post_ids_cast = $post_ids;
        \WP_Mock::userFunction('wp_list_pluck')
            ->once()
            ->andReturn($post_ids_cast);
    ...
    public function test_cache_clear_on_post_save(): void
    {
        ...
        /** @var array $taxonomies_cast */
        $taxonomies_cast = ['category', 'post_tag'];
        \WP_Mock::userFunction('get_object_taxonomies')
            ->once()
            ->with('post')
            ->andReturn($taxonomies_cast);
