 ...
 * @return void
 */
function doc_save_service_faqs_meta_box($post_id): void {
    // Validate post ID
    ...
    // Verify nonce
    if (!isset($_POST['doc_service_faqs_nonce']) ||
        !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['doc_service_faqs_nonce'])), 'doc_save_service_faqs')) {
        return;
    }
    ...
    if (count($faqs_answers) !== $count) {
        ...
        do_action('doc_faq_count_mismatch', $post_id, $count, count($faqs_answers));
    ...
    $new_faqs = apply_filters('doc_before_save_faqs', $new_faqs, $post_id);
    ...
    do_action('doc_after_save_faqs', $post_id, $new_faqs);
}
add_action('save_post', 'doc_save_service_faqs_meta_box');
