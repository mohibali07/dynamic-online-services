 ...
 */
function dynos_sanitize_cpt_settings(): array
{
	$options = \DynamicOnlineServices\Helpers\Options::get();
	$service_slug = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_post_type_slug', 'services');
	$taxonomy_slug = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_taxonomy_slug', 'service-category');
	$menu_position = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_menu_position', '');
	$menu_icon = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_menu_icon', 'dashicons-admin-customizer');

	// Custom Labels
	$cpt_singular = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_cpt_singular_name', 'Service');
	$cpt_plural = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_cpt_plural_name', 'Services');
	$tax_singular = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_tax_singular_name', 'Service Category');
	$tax_plural = \DynamicOnlineServices\Helpers\Options::get_option($options, 'service_tax_plural_name', 'Service Categories');

	// Sanitize slugs using centralized validation function
	$service_slug = dynos_validate_slug($service_slug, 'services');
	$taxonomy_slug = dynos_validate_slug($taxonomy_slug, 'service-category');

	// Validate menu position
	$menu_position = dynos_validate_menu_position($menu_position);

	// Validate menu icon
	$menu_icon = dynos_validate_menu_icon($menu_icon);
 ...
 */
function dynos_validate_menu_position($menu_position): ?int
{
	...
	if (null !== $menu_position && ($menu_position < 0 || $menu_position > 100)) {
		...
		$warning_message = sprintf(
			...
			esc_html($menu_position)
		);
		dynos_add_validation_warning_notice($warning_message, 'Invalid menu position ' . $menu_position . '. Using default.');
		$menu_position = null;
	}
 ...
 */
function dynos_validate_menu_icon($menu_icon): string
{
	...
	if (!preg_match('/^dashicons-[a-z0-9-]+$/i', $menu_icon)) {
		...
		dynos_add_validation_warning_notice($warning_message, 'Invalid menu icon format: ' . $menu_icon . '. Using default.');
		return 'dashicons-admin-customizer';
	}
