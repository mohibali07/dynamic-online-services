...
class FieldsSaver {
	...
	public static function init(): void {
		...
		add_action( 'created_' . $taxonomy_slug, array( __CLASS__, 'save' ), 10, 1 );
		add_action( 'edited_' . $taxonomy_slug, array( __CLASS__, 'save' ), 10, 1 );
	}
	...
	public static function save( $term_id ): void {
		...
		if ( isset( $_POST['service_cat_thumbnail'] ) ) {
			...
			if ( 0 === $thumbnail_id || empty( $thumbnail_id_raw ) ) {
				...
				$validated_thumbnail_id = HelperSanitization::validate_attachment_id( $thumbnail_id );
