 ...
 */
function doc_get_category_shortcode_child_categories($term, $hide_empty): array {
    $items = array();
    ...
    if (!is_wp_error($child_categories) && !empty($child_categories)) {
        foreach ($child_categories as $category) {
            ...
            if (is_wp_error($term_link)) {
                ...
                do_action('doc_category_content_term_link_error', $category, $term_link);
                continue;
            }
            ...
            $items[] = array(
                'type'        => 'category',
                'title'       => $category->name,
                'description' => wp_trim_words($category->description, DOC_DEFAULT_EXCERPT_LENGTH),
                'url'         => $term_link,
 ...
 */
function doc_get_category_shortcode_services($term, $atts): array {
    ...
    $query_args = apply_filters('doc_category_content_query_args', $query_args, $term);
    ...
    $services = apply_filters('doc_category_content_services', $services, $term);
    ...
    foreach ($services as $service_post) {
        ...
        $image_array = doc_get_service_banner_image($service_post->ID);
        $description = doc_get_service_description($service_post->ID);
        ...
        $items[] = array(
            ...
            'description' => wp_trim_words($description, DOC_DEFAULT_EXCERPT_LENGTH),
            'url'         => $permalink,
            'image_url'   => $image_url,
            'image_alt'   => $image_alt,
        );
