 ...
 */
function doc_validate_category_shortcode_attributes($atts): array {
    ...
    if (is_numeric($posts_per_page_raw)) {
        ...
        if ($atts['posts_per_page'] > DOC_MAX_POSTS_PER_PAGE) {
            $atts['posts_per_page'] = DOC_MAX_POSTS_PER_PAGE;
            doc_log_error(
                sprintf(
                    'posts_per_page value exceeded maximum (%d). Limited to %d for performance.',
                    DOC_MAX_POSTS_PER_PAGE,
                    DOC_MAX_POSTS_PER_PAGE
                ),
                'warning',
    ...
    if (!empty($atts['min_width'])) {
        $atts['min_width'] = doc_sanitize_css_value($atts['min_width'], 'min-width');
