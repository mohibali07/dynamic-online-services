 ...
 * @since 1.1.0
 */
if (!defined('doc_PAGINATION_RANGE')) {
    define('doc_PAGINATION_RANGE', 2);
}
 ...
 */
function doc_get_pagination_html($current_page, $total_pages): string {
    if ($total_pages <= 1) {
        return '';
    }
    ...
    if (is_wp_error($base_term_link)) {
        ...
        do_action('doc_pagination_term_link_error', $queried_object, $base_term_link);
        return '';
    }
    ...
    ob_start();
    ?>
    <nav class="sos-pagination" role="navigation" aria-label="<?php esc_attr_e('Pagination', 'dynamic-online-services'); ?>">
        <ul class="sos-pagination-list">
            ...
            // Page number links
            $range = doc_PAGINATION_RANGE; // Number of pages to show on each side of current page
