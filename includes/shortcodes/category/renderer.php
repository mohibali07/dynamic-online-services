 ...
 */
function doc_render_category_shortcode_items($items, $term, $atts): string {
    if (empty($items)) {
        $empty_message = apply_filters(
            'doc_category_content_empty_message',
    ...
    $grid_min_width = !empty($atts['min_width']) ? doc_escape_css_value($atts['min_width'], 'min-width') : doc_get_option($options, 'card_grid_min_width', DOC_DEFAULT_GRID_MIN_WIDTH);
    $grid_min_width = doc_escape_css_value($grid_min_width, 'min-width');
    if (empty($grid_min_width)) {
        $grid_min_width = DOC_DEFAULT_GRID_MIN_WIDTH;
    }
    
    $grid_column_gap = doc_get_option($options, 'card_grid_column_gap', '20px');
    $grid_row_gap = doc_get_option($options, 'card_grid_row_gap', '40px');
    
    // Handle columns attribute
    $columns = $atts['columns'];
    if ('auto' !== $columns && is_numeric($columns)) {
        $columns = absint($columns);
        $columns = min(max($columns, DOC_MIN_GRID_COLUMNS), DOC_MAX_GRID_COLUMNS);
        $grid_style = 'grid-template-columns: repeat(' . absint($columns) . ', 1fr);';
    } else {
        // $grid_min_width is already sanitized via doc_escape_css_value, safe for CSS
    ...
    $grid_column_gap = doc_escape_css_value($grid_column_gap, 'width');
    $grid_row_gap = doc_escape_css_value($grid_row_gap, 'height');
    ...
    $grid_class = apply_filters('doc_category_content_grid_class', 'service-card-grid', $term);
    
    // Build inline style using dedicated style builder helper
    $grid_style_value = doc_build_grid_style(array(
    ...
    foreach ($items as $index => $item) {
        echo doc_render_category_shortcode_item($item, $index, $term);
    }
 ...
 */
function doc_render_category_shortcode_item($item, $index, $term): string {
    $image_url = '';
    $image_alt = '';
    ...
    if (empty($image_url)) {
        $image_url = doc_get_placeholder_image_url();
    ...
    $item = apply_filters('doc_category_content_item', $item, $index, $term);
    ...
    $card_class = apply_filters('doc_category_content_card_class', 'service-card', $item, $index);
    $item_type = esc_attr($item['type']);
    $item_title = esc_html($item['title']);
    $item_url = esc_url($item['url']);
    $item_type_label = $item_type === 'category' ? _x('category', 'Item type label for accessibility', 'dynamic-online-services') : _x('service', 'Item type label for accessibility', 'dynamic-online-services');
    $aria_label = sprintf(
    ...
    echo '<a href="' . $item_url . '" class="service-card-image-link" aria-label="' . esc_attr($aria_label) . '">';
    ...
    echo '<h3 class="service-card-title"><a href="' . $item_url . '" aria-label="' . esc_attr($aria_label) . '">' . esc_html($item_title) . '</a></h3>';
    if (!empty($item['description'])) {
        echo '<p class="service-card-description">' . esc_html($item['description']) . '</p>';
    }
    $button_text = apply_filters('doc_category_content_button_text', __('VIEW DETAILS', 'dynamic-online-services'), $item);
    echo '<a href="' . $item_url . '" class="service-card-link" aria-label="' . esc_attr($aria_label) . '">' . esc_html($button_text) . '</a>';
    echo '</div></li>';
