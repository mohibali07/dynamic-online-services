...
class Cards
{
	...
	{
		// 1. Query Data - use cached version if available
		if (function_exists('dynos_get_cards_query_cached')) {
			$query = dynos_get_cards_query_cached($atts);
		} else {
			$query_args = dynos_get_cards_query_args($atts);
		...
		// Use Renderer Class
		$renderer = new \DynamicOnlineServices\Renderers\CardsRenderer();
	...
	protected function load_dependencies(): void
	{
		if (defined('DYNOS_PLUGIN_DIR')) {
			if (!function_exists('dynos_get_cards_query_args')) {
				require_once DYNOS_PLUGIN_DIR . 'includes/Shortcodes/cards/query.php';
			}
			// Renderer file is no longer required as we use the Class via Autoloader
			if (!function_exists('dynos_render_category_shortcode_item')) {
				// We might need this if the Renderer uses it as a helper/fallback.
				// CardsRenderer uses it. It is in includes/shortcodes/category/renderer.php
