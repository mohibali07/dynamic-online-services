 ...
 */
function doc_render_faqs_accordion($faqs, $post_id, $title = ''): string {
    // Enqueue accordion scripts and styles using dedicated helper
    doc_enqueue_faq_accordion_assets();

    // Allow filtering the title
    $title = apply_filters('doc_faqs_title', $title, $post_id);
    ...
    <div class="service-faqs-accordion" data-post-id="<?php echo esc_attr($post_id); ?>">
        ...
        <?php foreach ($faqs as $index => $faq) : ?>
            ...
            $faq_item = apply_filters('doc_faq_item', array(
                'question' => $question,
                'answer'   => $answer,
            ), $index, $post_id);
            ...
            <div class="faq-item" data-faq-index="<?php echo esc_attr($index); ?>">
                <h3 class="faq-question" id="faq-question-<?php echo esc_attr($index); ?>" role="button" tabindex="0" aria-expanded="false" aria-controls="faq-answer-<?php echo esc_attr($index); ?>">
                    <span class="faq-question-text"><?php echo esc_html($question); ?></span>
                    <span class="faq-icon" aria-hidden="true">+</span>
                </h3>
                <div class="faq-answer" id="faq-answer-<?php echo esc_attr($index); ?>" role="region" aria-labelledby="faq-question-<?php echo esc_attr($index); ?>">
