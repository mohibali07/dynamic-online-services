 ...
 *
 * @since 1.1.0
 * @param DYNOS_Exception $exception Exception object.
 * @param bool          $log_error Whether to log the error. Default true.
 * @return void
 */
function dynos_handle_exception(DYNOS_Exception $exception, bool $log_error = true): void
{
	if ($log_error) {
	...
	if (is_admin()) {
		...
		if (500 === $exception->get_http_status_code()) {
			...
			wp_die(
				...
				array(
					'response' => $exception->get_http_status_code(), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					'back_link' => true,
				)
			);
		}
...
function dynos_handle_activation_error(string $message, string $title = '', bool $back_link = true): void
{
	...
	// Create exception
	$exception = new DYNOS_Exception($message, 'activation_error', 0, array(), 500);
	...
	wp_die(
		...
		array(
			'response' => 500,
			'back_link' => $back_link, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		)
	);
}
...
function dynos_handle_permission_error(string $message = '', string $capability = 'manage_options'): void
{
	...
	// Create exception
	$exception = new DYNOS_Exception($message, 'permission_error', 403, array('capability' => $capability), 403);
