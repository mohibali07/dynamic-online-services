    ...
    public static function sanitize(array $options): array
    {
        ...
        foreach ($dimension_fields as $field) {
            if (isset($options[$field])) {
                ...
                if (function_exists('doc_sanitize_css_dimension')) {
                    $sanitized[$field] = doc_sanitize_css_dimension($options[$field]);
                } else {
                    $sanitized[$field] = sanitize_text_field($options[$field]);
                }
        ...
        foreach ($transform_fields as $field) {
            if (isset($options[$field])) {
                if (function_exists('doc_sanitize_css_transform')) {
                    $sanitized[$field] = doc_sanitize_css_transform($options[$field]);
                } else {
                    $sanitized[$field] = sanitize_text_field($options[$field]);
                }
        ...
        foreach ($box_shadow_fields as $field) {
            if (isset($options[$field])) {
                if (function_exists('doc_sanitize_css_box_shadow')) {
                    $sanitized[$field] = doc_sanitize_css_box_shadow($options[$field]);
                } else {
                    $sanitized[$field] = sanitize_text_field($options[$field]);
                }
        ...
        foreach ($slug_fields as $field => $default_slug) {
            if (isset($options[$field])) {
                if (function_exists('doc_validate_slug')) {
                    $sanitized[$field] = doc_validate_slug($options[$field], isset($defaults[$field]) ? $defaults[$field] : $default_slug);
                } else {
                    $sanitized[$field] = sanitize_title($options[$field]);
                }
            } else {
