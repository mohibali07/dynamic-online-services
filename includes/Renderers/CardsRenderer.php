...
class CardsRenderer implements RendererInterface
{
	...
	public function render(array $data): string
	{
		...
		foreach ($items as $index => $item) {
			...
			if (function_exists('dynos_render_category_shortcode_item')) {
				...
				echo $this->render_fallback_card($item);
			}
		}
	...
	{
		$image_url_fn = 'dynos_get_placeholder_image_url';
		$image_url = !empty($item['image_url']) ? $item['image_url'] : (function_exists($image_url_fn) ? $image_url_fn() : '');
