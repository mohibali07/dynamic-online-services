 ...
 */
function doc_sanitize_cpt_settings(): array {
    $options = doc_get_options();
    $service_slug = doc_get_option($options, 'service_post_type_slug', 'services');
    $taxonomy_slug = doc_get_option($options, 'service_taxonomy_slug', 'service-category');
    $menu_position = doc_get_option($options, 'service_menu_position', '');
    $menu_icon = doc_get_option($options, 'service_menu_icon', 'dashicons-admin-customizer');

    // Sanitize slugs using centralized validation function
    $service_slug = doc_validate_slug($service_slug, 'services');
    $taxonomy_slug = doc_validate_slug($taxonomy_slug, 'service-category');

    // Validate menu position
    $menu_position = doc_validate_menu_position($menu_position);

    // Validate menu icon
    $menu_icon = doc_validate_menu_icon($menu_icon);
 ...
 */
function doc_validate_menu_position($menu_position): ?int {
    $menu_position = !empty($menu_position) ? absint($menu_position) : null;
    
    if ($menu_position !== null && ($menu_position < 0 || $menu_position > 100)) {
        ...
        doc_add_validation_warning_notice($warning_message, 'Invalid menu position ' . $menu_position . '. Using default.');
        $menu_position = null;
    }
 ...
 */
function doc_validate_menu_icon($menu_icon): string {
    ...
    if (!preg_match('/^dashicons-[a-z0-9-]+$/i', $menu_icon)) {
        ...
        doc_add_validation_warning_notice($warning_message, 'Invalid menu icon format: ' . $menu_icon . '. Using default.');
        return 'dashicons-admin-customizer';
    }
