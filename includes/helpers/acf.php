 ...
 */
function doc_get_acf_field($field_name, $post_id = false, $default = ''): mixed {
    // Validate field name
    ...
    if (!function_exists('get_field')) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG && !has_action('admin_notices', 'doc_acf_missing_notice')) {
            add_action('admin_notices', 'doc_acf_missing_notice');
        }
        return $default;
    }
...
/**
 * Alias for doc_get_acf_field for backward compatibility.
 *
 * @since 1.1.0
 ...
 */
function doc_get_field($selector, $post_id = false): mixed {
    return doc_get_acf_field($selector, $post_id, false);
}
 ...
 * @return void
 */
function doc_acf_missing_notice(): void {
