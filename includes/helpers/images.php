 ...
 */
function doc_get_placeholder_image_url($size = 'full'): string {
    // Allow filtering the placeholder URL
    $placeholder_url = apply_filters('doc_placeholder_image_url', '', $size);
    ...
    $local_placeholder = DOC_PLUGIN_URL . 'assets/images/placeholder.png';
    $local_path = DOC_PLUGIN_DIR . 'assets/images/placeholder.png';
 ...
 */
function doc_get_service_banner_image($post_id): array {
    ...
    $image_array = doc_get_acf_field('banner_bg_image', $post_id, array());
 ...
 */
function doc_get_service_description($post_id): string {
    $post_id = absint($post_id);
    if (0 === $post_id) {
        return '';
    }
    ...
    $description = doc_get_acf_field('banner_description', $post_id, '');
