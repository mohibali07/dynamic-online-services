...
class Acf
{
	 ...
	 */
	public static function get_field(string $field_name, $post_id = false, $default = ''): mixed
	{
		// Validate field name
		...
		if (!function_exists('get_field')) {
			...
			if (defined('WP_DEBUG') && WP_DEBUG && !has_action('admin_notices', [self::class, 'show_missing_notice'])) {
				add_action('admin_notices', [self::class, 'show_missing_notice']);
			}
			return $default;
		}
	...
	/**
	 * Alias for get_field for backward compatibility.
	 *
	 * @since 1.1.0
	 ...
	 */
	public static function get_field_alias(string $selector, $post_id = false): mixed
	{
		return self::get_field($selector, $post_id, false);
	}
	 ...
	 * @return void
	 */
	public static function show_missing_notice(): void
	{
