...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <div {...blockProps}>
            <InspectorControls>
                <PanelBody title={__('Settings', 'dynamic-online-services')}>
                    <TextControl
                        label={__('Category (Slug or ID)', 'dynamic-online-services')}
                        value={attributes.category}
                        onChange={(category) => setAttributes({ category })}
                        help={__('Comma separated slugs or IDs.', 'dynamic-online-services')}
                    />
                    <TextControl
                        label={__('Post IDs', 'dynamic-online-services')}
                        value={attributes.ids}
                        onChange={(ids) => setAttributes({ ids })}
                        help={__('Specific post IDs to include.', 'dynamic-online-services')}
                    />
                    <RangeControl
                    ...
                    />
                    <ToggleControl
                        label={__('Show Pagination', 'dynamic-online-services')}
