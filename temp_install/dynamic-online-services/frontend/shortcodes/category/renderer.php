...
function dynos_render_category_shortcode_items($items, $term, $atts): string
{
	...
	$grid_min_width = !empty($atts['min_width']) ? \TechmireSolutions\DynamicOnlineServices\Helpers\Sanitization::escape_css_value($atts['min_width'], 'min-width') : \TechmireSolutions\DynamicOnlineServices\Helpers\Options::get_option($options, 'card_grid_min_width', \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_grid_min_width());
	...
	if (empty($grid_min_width)) {
		$grid_min_width = \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_grid_min_width();
	}
	...
	if ('auto' !== $columns && is_numeric($columns)) {
		...
		$columns = min(max($columns, \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_min_grid_columns()), \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_max_grid_columns());
