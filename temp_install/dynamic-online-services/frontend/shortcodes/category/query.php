	...
	if ( ! is_wp_error( $child_categories ) && ! empty( $child_categories ) ) {
		...
		foreach ( $child_categories as $category ) {
			...
			$items[] = array(
				'type'        => 'category',
				'title'       => $category->name,
				'description' => wp_trim_words( $category->description, \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_default_excerpt_length() ),
				'url'         => $term_link,
...
function dynos_get_category_shortcode_services( $term, $atts ): array {
	...
	foreach ( $services as $service_post ) {
		...
		$items[] = array(
			...
			'description' => wp_trim_words( $description, \TechmireSolutions\DynamicOnlineServices\Core\Configuration::get_default_excerpt_length() ),
			'url'         => $permalink,
			'image_url'   => $image_url,
			'image_alt'   => $image_alt,
		);
